...
echo "rgb"
/usr/local/gdal/gdal-dev/bin/gdal_translate --config OGR_SQLITE_SYNCHRONOUS OFF -co APPEND_SUBDATASET=YES -co RASTER_TABLE=ch.so.agi.orthofoto.2015-bl.rgb -co TILE_FORMAT=PNG_JPEG -of GPKG /opt/Geodaten/ch/so/kva/orthofoto/2015_bl/rgb/12_5cm/ortho2015rgb.vrt /opt/Geodaten/ch/so/kva/orthofoto/orthofoto.gpkg

/usr/local/gdal/gdal-dev/bin/gdaladdo --config OGR_SQLITE_SYNCHRONOUS OFF -oo TABLE=ch.so.agi.orthofoto.2015-bl.rgb -r average /opt/Geodaten/ch/so/kva/orthofoto/$GPKG 2 4 8 16 32 64 128 256 512
